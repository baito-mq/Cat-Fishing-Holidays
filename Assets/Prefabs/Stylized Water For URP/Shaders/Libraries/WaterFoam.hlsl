...
//TODO: use shader feature here so no 2 outputs need to be calculated
...
{
    // half2 shoreline_uv = half2(uv.x, depth);
    ByDepth = 0;

    // ByDepth.xy = ShoreMovement(uv, movement.y, 1/tilingandoffset.z);
    // ByDepth.zw = ShoreMovement(uv, movement.w, 1/tilingandoffset.w);
...
{
    //Out = lerp(foamcolor1, foamcolor2, foam1 * foamcolor1.w);
    ...
    //Out = primary_foam * foamcolor1 + (1-primary_foam) * secondary_foam * foamcolor2;
}
...
void SurfaceFoamColorShadows_half(half4 color, half shadow)
{
